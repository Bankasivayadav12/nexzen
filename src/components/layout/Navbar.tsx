...
export default function Navbar({ onOpenEnquireModal }: NavbarProps) {
  ...
  return (
    <header className="sticky top-0 z-50 w-full pt-1.5 sm:pt-3 px-2 sm:px-6 lg:px-8 transition-all duration-300">
      <div
        className={`max-w-7xl mx-auto rounded-full transition-all duration-300 ${
          scrolled
            ? "bg-white/95 backdrop-blur-xl border border-white/80 shadow-[0_10px_30px_-10px_rgba(9,31,56,0.12)] py-1.5 sm:py-2.5 px-3 sm:px-6"
            : "bg-white/90 backdrop-blur-md border border-white/60 shadow-[0_4px_20px_-5px_rgba(9,31,56,0.06)] py-2 sm:py-3 px-3 sm:px-6"
        }`}
      >
        <div className="flex items-center justify-between gap-2 sm:gap-6">
          {/* Brand Logo & Asset Name */}
          <Link href="/" className="flex items-center gap-1.5 sm:gap-2.5 group shrink-0">
            <div className="relative overflow-hidden rounded-lg sm:rounded-xl p-0.5 sm:p-1 bg-gradient-to-br from-blue-50 to-amber-50 border border-blue-100/60 transition-transform duration-300 group-hover:scale-105">
              <img
                src="/logo.png"
                alt="NexZen Asset"
            ...
            <div className="flex flex-col leading-none">
              <div className="flex items-center gap-0.5 sm:gap-1">
                <span className="text-xs sm:text-base lg:text-xl font-black tracking-wider text-[#091F38] uppercase">
                  NEXZEN
                </span>
                <span className="text-xs sm:text-base lg:text-xl font-black tracking-wider text-[#C69214] uppercase">
                  ASSET
                </span>
              </div>
              <span className="text-[8px] sm:text-[9px] font-bold text-slate-500 tracking-widest uppercase hidden sm:block">
          ...
          <div className="flex items-center space-x-2 sm:space-x-3 shrink-0">
            ...
            <button
              ...
              className="lg:hidden p-1.5 sm:p-2 text-slate-800 hover:text-[#0066FF] focus:outline-none rounded-full hover:bg-slate-100 transition-colors"
